...
def out_netcdf_data(filename,grid,ntimes,locRad,recon_times,Xam_pp_ensemble,truth_latlon,Xbm_pp_ensemble):
    """
...
def analysis_state_generator(cfg,locRad,prox_manager,grid,nproxies,ntimes,nens,Xb_one_new,truth_latlon,white_noise_latlon,Xb_sampler,var_noise_latlon):
    """
...
locRad = [3000,5000,7000,9000,15000,20000,25000]
nite=len(locRad)
...
cfile = 'config_season_GPCC_COOL_locRad.yml'
...
for iter_num in range(0,iter):
    ...
    for i in range(len(locRad)):
        Xam_pp= analysis_state_generator(cfg,locRad[i],prox_manager,grid,nproxies,ntimes,prior,Xb_one_new,truth_latlon,add_noise_latlon,Xb_sampler,var_noise_latlon)#the analysis state (nlat*nlon,Ny)   
    ...
    out_netcdf_data(filename,grid,ntimes,locRad,recon_times,Xam_pp_ensemble,truth_latlon,Xbm_pp_ensemble)
