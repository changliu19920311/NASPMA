...
@class_docs_fixer
class GradePSM(BasePSM):
    def __init__(self, config, proxy_obj, psm_data=None, calib_obj=None):
        self.psm_key = 'grade'

        proxy = proxy_obj.type
        site = proxy_obj.id
        r_crit = config.psm.grade.psm_r_crit
        self.lat  = proxy_obj.lat
        self.lon  = proxy_obj.lon
        self.elev = proxy_obj.elev
        self.datatag_calib = config.psm.grade.datatag_calib
        self.avgPeriod = config.psm.grade.avgPeriod
        ...
        try:
            ...
            self.intercept = psm_site_data['PSMintercept']
            self.R = psm_site_data['PSMmse']
            self.Threshold_1 = psm_site_data['Threshold_1']
            self.Threshold_2 = psm_site_data['Threshold_2']
            self.Threshold_3 = psm_site_data['Threshold_3']
            self.Threshold_4 = psm_site_data['Threshold_4']
            print('psm_data',self.Threshold_4)
            # check if seasonality defined in the psm data
            # if it is, return as an attribute
            if 'Seasonality' in list(psm_site_data.keys()):
                self.seasonality = psm_site_data['Seasonality']
            ...
            if calib_obj is None:
                #print 'calibration object does not exist ...creating it...'
                # TODO: Fix call and Calib Module                
                datag_calib = config.psm.grade.datatag_calib
                calib_obj= LMR_calibrate.calibration_assignment(datag_calib)
                calib_obj.datadir_calib = config.psm.grade.datadir_calib
                calib_obj.read_calibration()
    ...
    def basic_psm(self, data):
        ...
        for i in range(len(data)):
            if data[i] <= self.Threshold_1: 
                data[i] = 5
            elif data[i] <= self.Threshold_2:
                data[i] = 4
            elif data[i] <= self.Threshold_3:
                data[i] = 3
            elif data[i] <= self.Threshold_4:
                data[i] = 2
            else:
                data[i] = 1
    ...
    def calibrate(self, C, proxy, diag_output=False, diag_output_figs=False):
        ...
        reg_x = np.zeros(shape=[nbcyears])
        reg_x[:] = np.nan # initialize with nan's
        Pre_season = np.zeros(shape=[nbcyears])
        Pre_season[:] = np.nan # initialize with nan's
        ...
        sigma=np.std(Pre_season, ddof = 1)
        Mean=np.nanmean(Pre_season)
        q = [Mean-1.28*sigma,Mean-0.52*sigma,Mean+0.52*sigma,Mean+1.28*sigma]

        for i in range(nbcyears):
            if Pre_season[i] <= q[0]: 
                reg_x[i] = 5
            elif Pre_season[i] <= q[1]:
                reg_x[i] = 4
            elif Pre_season[i] <= q[2]:
                reg_x[i] = 3
            elif Pre_season[i] <= q[3]:
                reg_x[i] = 2
            else:
                reg_x[i] = 1
        ...
        self.Threshold_1   = q[0]
        self.Threshold_2   = q[1]
        self.Threshold_3   = q[2]
        self.Threshold_4   = q[3]
        ...
        if diag_output:
            ...
            if diag_output_figs:
                ...
                plt.savefig('proxy_%s_%s_GradeModel_calib.png' % (
                    proxy.type.replace(" ", "_"), proxy.id),
                              bbox_inches='tight')
                plt.close()
    ...
    def get_kwargs(config):
        try:
            psm_data = GradePSM._load_psm_data(config)
            return {'psm_data': psm_data}
        except IOError as e:
            print(e)
            return {}
    ...
    def _load_psm_data(config):
        ...
        pre_calib_file = config.psm.grade.pre_calib_datafile
...
_psm_classes = {'linear': LinearPSM, 'linear_TorP': LinearPSM_TorP,
                ...
                'bayesreg_uk37': BayesRegUK37PSM,'grade': GradePSM}
