...
def out_netcdf_data(filename,grid,ntimes,recon_times,Xam_pp_ensemble,Xav_pp_ensemble,truth_latlon,Xbm_pp_ensemble):
    """
    ...
    time.units = 'Year'
    time.long_name = 'time'
    pr_sfc_Amon_analysis = ncfile.createVariable('pr_sfc_Amon_analysis',np.float64,('lat','lon','time')) # note: unlimited dimension is leftmost
    pr_sfc_Amon_analysis.units = 'C' # degrees Kelvin
    pr_sfc_Amon_analysis.standard_name = 'analysis_surface_precipitatio_anomaly' 
    # this is a CF standard name
    pr_sfc_Amon_analysis_var = ncfile.createVariable('pr_sfc_Amon_analysis_var',np.float64,('lat','lon','time')) # note: unlimited dimension is leftmost
    pr_sfc_Amon_analysis_var.units = 'C' # degrees Kelvin
    pr_sfc_Amon_analysis_var.standard_name = 'analysis_surface_precipitatio_anomaly_var'
    ...
    pr_sfc_Amon_prior = ncfile.createVariable('pr_sfc_Amon_prior',np.float64,('lat','lon')) # note: unlimited dimension is leftmost
    ...
    pr_sfc_Amon_analysis_var[:,:,:] = Xav_pp_ensemble
...
def analysis_state_generator(cfg,prox_manager,grid,nproxies,ntimes,nens,Xb_one_new,truth_latlon,white_noise_latlon,Xb_sampler,var_noise_latlon):
   ...
   loc : localization
    ...
    Xav_pp=np.zeros([grid.nlat*grid.nlon,ntimes])
    # -------------------------------------
    ...
    for yr_idx, t in enumerate(range(core.recon_period[0], core.recon_period[1]+1, core.recon_timescale)):
            ...
            xam_pp,xap_pp,xav_pp= Kalman_ESRF(cfg,vY[index,yr_idx],vR[index],vYe[index,:],Xb_one_new,Ye_assim_coords,loc)
            Xam_pp[:,yr_idx]=xam_pp
            Xav_pp[:,yr_idx]=xav_pp
           # filename = core.datadir_output+truth_var+"_"+cfile+"_"+str(i)+"_"+str(t)+".nc"
           # out_Xap_netcdf_data(filename,grid,xap_pp)
            
    return Xam_pp,Xav_pp
...
def Kalman_ESRF(cfg,vY,vR,vYe,Xb_in, Ye_assim_coords,loc):
    ...
    xav = np.var(Xa[0:nx,:],axis=1,ddof=1)
    return xam,Xap[0:nx,:],xav
...
cfile = 'config_season_GPCC_WARM.yml'
...
Xam_pp_ensemble=np.zeros([grid.nlat,grid.nlon,ntimes])
Xbm_pp_ensemble=np.zeros([grid.nlat,grid.nlon])
p_tru = p[nsample:]
for iter_num in range(0,iter):
    print('iter_num',iter_num)
    itr_str = 'r{:d}'.format(iter_num)
    pp = np.random.RandomState(seed=iter_num).permutation(len(p_tru))
    print(pp[0:prior])
    Xb_one_new = Xb_one[:,pp[0:prior]] # the priors. it is required that the truth_vector and 
    ...
    Xb_sampler = np.reshape(Xb_one_new,[grid.nlat,grid.nlon,prior])
   ...
   # assimilating 
    ...
    Xam_pp,Xav_pp= analysis_state_generator(cfg,prox_manager,grid,nproxies,ntimes,prior,Xb_one_new,truth_latlon,add_noise_latlon,Xb_sampler,var_noise_latlon)#the analysis state (nlat*nlon,Ny)   
    Xam_pp_latlon=np.reshape(Xam_pp,[grid.nlat,grid.nlon,ntimes]) #the analysis sta at the lat/lon grid (nlat,nlon,Ny)
    Xam_pp_ensemble=Xam_pp_latlon
    Xav_pp_latlon=np.reshape(Xav_pp,[grid.nlat,grid.nlon,ntimes]) #the analysis sta at the lat/lon grid (nlat,nlon,Ny)
    Xav_pp_ensemble=Xav_pp_latlon
    Xbm_pp_ensemble=Xbm_pp_latlon
    filename=core.datadir_output+truth_var+"_"+cfile+"_"+itr_str+".nc"
    out_netcdf_data(filename,grid,ntimes,recon_times,Xam_pp_ensemble,Xav_pp_ensemble,truth_latlon,Xbm_pp_ensemble)