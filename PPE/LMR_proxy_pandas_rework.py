...
@class_docs_fixer
class ProxySTR_WARM(BaseProxyObject):
    @staticmethod
    def get_psm_obj(config,proxy_type):
        psm_key = config.proxies.STR_WARM.proxy_psm_type[proxy_type]
        return LMR_psms.get_psm_class(psm_key)
    ...
    def load_site(cls, config, site, data_range=None, meta_src=None,
        ...
        STR_WARM_cfg = config.proxies.STR_WARM
        if meta_src is None:
            meta_src = load_data_frame(STR_WARM_cfg.metafile_proxy)
        if data_src is None:
            data_src = load_data_frame(STR_WARM_cfg.datafile_proxy)
            data_src = data_src.to_dense()
        ...
        print(end_yr)
        lat = site_meta['Lat (N)'].iloc[0]
        proxy_type = site_meta['Archive type'].iloc[0]
        ...
        # transform in "anomalies" (time-mean removed) if option activated
        if config.proxies.STR_WARM.proxy_timeseries_kind == 'anom':
            values = values - values.mean()
    ...
    def load_all(cls, config, data_range, meta_src=None,
        ...
        # Load source data files
        if meta_src is None:
            meta_src = load_data_frame(config.proxies.STR_WARM.metafile_proxy)
        if data_src is None:
            data_src = load_data_frame(config.proxies.STR_WARM.datafile_proxy)
            data_src = data_src.to_dense()

        filters = config.proxies.STR_WARM.simple_filters
        proxy_order = config.proxies.STR_WARM.proxy_order
        ptype_filters = config.proxies.STR_WARM.proxy_assim2
        dbase_filters = config.proxies.STR_WARM.database_filter
        availability_filter = config.proxies.STR_WARM.proxy_availability_filter
        availability_fraction = config.proxies.STR_WARM.proxy_availability_fraction
...
@class_docs_fixer
class ProxySTR_COOL(BaseProxyObject):

    @staticmethod
    def get_psm_obj(config,proxy_type):

        psm_key = config.proxies.STR_COOL.proxy_psm_type[proxy_type]
    ...
    def load_site(cls, config, site, data_range=None, meta_src=None,
        ...
        STR_COOL_cfg = config.proxies.STR_COOL
        if meta_src is None:
            meta_src = load_data_frame(STR_COOL_cfg.metafile_proxy)
        if data_src is None:
            data_src = load_data_frame(STR_COOL_cfg.datafile_proxy)
            data_src = data_src.to_dense()
        ...
        # transform in "anomalies" (time-mean removed) if option activated
        if config.proxies.STR_COOL.proxy_timeseries_kind == 'anom':
            values = values - values.mean()
    ...
    def load_all(cls, config, data_range, meta_src=None,
        ...
        # Load source data files
        if meta_src is None:
            meta_src = load_data_frame(config.proxies.STR_COOL.metafile_proxy)
        if data_src is None:
            data_src = load_data_frame(config.proxies.STR_COOL.datafile_proxy)
            data_src = data_src.to_dense()

        filters = config.proxies.STR_COOL.simple_filters
        proxy_order = config.proxies.STR_COOL.proxy_order
        ptype_filters = config.proxies.STR_COOL.proxy_assim2
        dbase_filters = config.proxies.STR_COOL.database_filter
        availability_filter = config.proxies.STR_COOL.proxy_availability_filter
        availability_fraction = config.proxies.STR_COOL.proxy_availability_fraction
...
_proxy_classes = {'PAGES2kv1': ProxyPAGES2kv1, 'LMRdb': ProxyLMRdb, 'NCDCdtda': ProxyNCDCdtda, 'STR_WARM': ProxySTR_WARM, 'STR_COOL': ProxySTR_COOL}
