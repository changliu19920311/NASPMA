...
def NA_zoom(var_array_full,coords_array_full):
    """
    Truncate lat,lon grid to another resolution using local distance-weighted 
    averages. 
    ...
    Originator: Chang liu
                University of arkansas
               Feb 2020
    """
        
    lat_bnds = [ 0 , 80 ]
    lon_bnds = [ 220, 310 ] # degrees east ?  
    lats = coords_array_full[:,0] 
    lons = coords_array_full[:,1] 

    coords_inds = np.where((lats > lat_bnds[0]) & (lats < lat_bnds[1])&(lons > lon_bnds[0]) & (lons < lon_bnds[1]))
    X_center=var_array_full[coords_inds,:]
    X_new = X_center[0,:,:]
    if np.isnan(X_new).any():
        X_new = np.ma.masked_invalid(X_new)
    nlat=np.size(np.unique(lats[coords_inds]))
    nlon=np.size(np.unique(lons[coords_inds]))
    print("nlat",nlat)
    print("nlon",nlon)
    print('X_new',X_new.shape)
    print(lats[coords_inds].shape)
    lat_new = lats[coords_inds].reshape(nlat,nlon).T.T
    lon_new = lons[coords_inds].reshape(nlat,nlon)
...
def create_precalc_ye_filename(config,psm_key,prior_kind):
    ...
    if psm_key == 'linear':
        ...
        calib_avgPeriod = config.psm.grade.avgPeriod
        calib_str = config.psm.grade.datatag_calib+calib_str_ext
        state_vars_for_ye = config.psm.grade.psm_required_variables
    ...
    elif proxy_str == 'STR_WARM':
        proxy_str = proxy_str 
    elif proxy_str == 'STR_COOL':
        proxy_str = proxy_str      
...
def load_precalculated_ye_vals_psm_per_proxy(config, proxy_manager, proxy_set, sample_idxs):
    ...
    for psm_key in psm_keys:
        ...
        if psm_key == 'linear':
            ...
            pkind = list(config.psm.grade.psm_required_variables.values())[0]
        elif psm_key == 'h_interp':
...
def validate_config(config):
    ...
    if proxy_database == 'LMRdb':
        ...
        proxy_cfg = config.proxies.STR_WARM
    elif proxy_database == 'STR_COOL':
        proxy_cfg = config.proxies.STR_COOL
    else:
