...
class ProxyLMRdb(BaseProxyObject):
    ...
    def load_all(cls, config, data_range, meta_src=None,
        ...
        print(config.proxies.LMRdb.proxy_blacklist)
        NA_zoom = config.proxies.LMRdb.NA_zoom       
        ...
        region_mask = meta_src[meta_src.columns[0]] == 0
        region_mask |= True
        ...
        # the reconstruction period (recon_period in configuration, or
        # data_range here).
        if NA_zoom: # if not None
            lat = meta_src['Lat (N)'] #.array
            lon = meta_src['Lon (E)'] #.array
            #if lon < 0:
            #    lon += 360.
            print(lon.tolist())
        ...
        if proxy_blacklist:
            ...
            for pbl in proxy_blacklist:
                tmp = meta_src['Archive type'].map(lambda x: x.startswith(pbl))
                inds = meta_src['Archive type'][tmp].index

                for ni in inds:
                    if region_mask[ni]:
                        blacklist_mask[ni] = False
                    else:
                        blacklist_mask[ni] = True
...
_proxy_classes = { 'LMRdb': ProxyLMRdb}
