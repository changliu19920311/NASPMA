...
def make_plot(vplot,grid,figsize=10,savefig=None,vmax=None):
    ...
    ax = plt.axes(projection=ccrs.Robinson(central_longitude=-90.),zorder=8)
...
def make_plot_NA(vplot,grid,figsize=10,savefig=None,vmax=None):
    ...
    ax = plt.axes(projection=ccrs.Robinson(central_longitude=-90.),zorder=8)
    ax.set_extent([NA_west, NA_east, NA_south, NA_north])
    ax.coastlines()
    #cs = ax.contourf(lon,lat,vplot,transform=ccrs.PlateCarree(),cmap='bwr')
    if vmax:
        maxv = vmax
...
def load_prior(cfg,verbose=False):
    ...
    print('avgInterval',prior.avgInterval)
...
def load_analyses(cfg,full_field=False,lmr_gm=None,lmr_time=None,satime=1900,eatime=1999,svtime=1880,evtime=1999):
    ...
    if not full_field:
        try:
            ...
            npzfile = np.load(filen,allow_pickle = True)
...
def prior_regrid(cfg,X,Xb_one,verbose=False):
    ...
    for var in list(X.full_state_info.keys()):
        ...
        if X.full_state_info[var]['vartype'] == '2D:horizontal':
            ...
            elif regrid_method == 'NA_zoom':
                [var_array_new, lat_new, lon_new] = \
                    LMR_utils.NA_zoom(var_array_full, coords_array_full)
